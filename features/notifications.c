...
void feature_notifications_host_resolve(struct host_ctx *ctx)
{
    if (!ctx->x11)
        return;
    feature_require_executable("/usr/bin/dunst", "dunst", "notifications",
                               "dunst");
}

static void setup_guest_notifications(const struct guest_config *cfg,
                                      int log_fd)
{
    char *session_bus =
        xasprintf("--bus=unix:path=/run/user/%u/bus", cfg->header.uid);
    ...
    if (!feature_wait_for_guest_bus_name(
            session_bus, "org.freedesktop.Notifications", GUEST_SERVICE_DUNST,
            "dunst", (uid_t)cfg->header.uid, (gid_t)cfg->header.gid))
